...
class StateDTSPMS(NamedTuple):
    ...
    def num_items_by_stack(self):
        ...
        """
        return (self.items_in_stack > 0).sum(dim=-1)
    ...
    def num_items(self):
        ...
        """
        return self.num_items_by_stack.sum(dim=-2)
    ...
    def initialize(input, visited_dtype=torch.uint8):
        ...
        return StateDTSPMS(
            ...
            # Add step dimension
            items_in_stack=torch.torch.zeros((batch_size, 1, num_stacks, stack_size), dtype=torch.long, device=pickup_loc.device),
            visited_= (  
    ...
    @staticmethod
    def _get_top_item(stack):
        """
        Get the top item on each stack
        
        :return:            item: (B, step, num_stacks) of top item on every stack
                            index: (B, step, num_stacks) of the index used to select item
        """
        stack_next_empty_idx = self.num_items_by_stack()[:, :, None]
        # shape (B, step, 1) of the top empty position on selected stacks
        
        # reshape to (B, step, num_stacks, 1)
        stack_next_empty_idx = stack_next_empty_idx[:,:,:,None]
        top_items = stack.gather(dim = -1, index = stack_next_empty_idx)
        # reshape both outputs to (B, step, num_stacks)
        return top_items[:,:,:,0], stack_next_empty_idx[:,:,:,0]
    
    @staticmethod
    def _pop_item(stack, stack_index):
        """
        :param stack:       torch.Tensor (B, step, num_stacks, stack_size) of stack contents
        :param stack_index: torch.Tensor (B, step) of which stack to select
        
        :return:            (B, step, 1) of items popped from stack
        """
        stack = stack[:,:,None]
        top_items, indices = self._get_top_item()
        top_item_on_stack = top_items.gather(dim = -1, index = stack)
        
    @staticmethod
    def _push_item(items, stack):
        pass
    ...
    def update(self, selected, stack):
        ...
        :param selected:    torch.Tensor of shape (B, step), 
                            Entries in (0, 1, ..., N-1) representing which node to travel to next
                            
        :param stack:       torch.Tensor of shape (B, step), 
